...
// SPAWN NEW CUE BALL USING SPACEBAR
...
void Game::Load()
{
	...
	BallInner* BotLeftPockColl = new BallInner(15, ParametersPocketsBotLeft, vec3(0, 0, 0));
	...
	BallInner* MidLeftPockColl = new BallInner(15, ParametersPocketsMidLeft, vec3(0, 0, 0));
	...
	BallInner* TopLeftPockColl = new BallInner(15, ParametersPocketsTopLeft, vec3(0, 0, 0));
	...
	BallInner* TopRightPockColl = new BallInner(15, ParametersPocketsTopRight, vec3(0, 0, 0));
	...
	BallInner* MidRightPockColl = new BallInner(15, ParametersPocketsMidRight, vec3(0, 0, 0));
	...
	BallInner* BotRightPockColl = new BallInner(15, ParametersPocketsBotRight, vec3(0, 0, 0));
	...
	PhysicsParameters* ParametersCueBall = new PhysicsParameters(10, vec2(500, 250), vec2(0, 2000), 0.8f);
	CueBall = new Balls(25, ParametersCueBall, vec3(1, 1, 1));	  //White
...
void Game::SpawnCue()
{
	dynObjs.erase(dynObjs.end() - 1);
	vecX = rand() %  1001 + (-500);
	vecY = rand() % 2000;
	Drag* DragCueBall = new Drag(10.0f, 0.0f);
	PhysicsParameters* ParametersCueBall = new PhysicsParameters(10, vec2(500, 250), vec2(vecX, vecY), 0.8f);
	CueBall = new Balls(25, ParametersCueBall, vec3(1, 1, 1));	  
	forceRegistry->Add(CueBall, DragCueBall);
	dynObjs.push_back(CueBall);
}
...
{	
	...
	for (vector<PhysicsEntity*>::iterator it = dynObjs.begin(); it < dynObjs.end();)
	{
		if (!(*it)->getCollider()->isInGame)
		{
			it = dynObjs.erase(it);
			cout << "DELETED" << endl;
		}
